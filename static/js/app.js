...
async function sendQuery() {
    ...
    try {
        ...
        addMessageToChat('agent', responseText, queryType);

        // Offer contextual follow-ups to keep the conversation flowing
        renderFollowUps(result, queryType);
...
// Render contextual follow-up chips under the latest assistant message
function renderFollowUps(result, queryType) {
    try {
        const agentRes = (result && result.result && result.result.agent_responses) || {};
        const suggestions = [];

        // Generic suggestions
        suggestions.push('Find common free time on Monday');
        suggestions.push('Show today\'s commitments for Agent 1');
        suggestions.push('Summarize both users\' afternoon availability');

        // If we saw zero results, suggest adding data
        const zeroData = Object.values(agentRes).every(r => !r.relevant_data || !r.relevant_data.documents || !r.relevant_data.documents[0] || r.relevant_data.documents[0].length === 0);
        if (zeroData) {
            suggestions.unshift('Add a new schedule: "Meeting 3-4 PM"');
        }

        const chatMessages = document.getElementById('chat-messages');
        const lastAgentMsg = chatMessages.querySelector('.chat-agent:last-child .followups');
        if (!lastAgentMsg) return;

        lastAgentMsg.innerHTML = suggestions.slice(0, 3).map(s => `<button class="followup-chip" onclick="quickAsk('${s.replace(/'/g, "\\'")}')">${s}</button>`).join('');
    } catch (e) {
        console.warn('Follow-ups render error:', e);
    }
}
